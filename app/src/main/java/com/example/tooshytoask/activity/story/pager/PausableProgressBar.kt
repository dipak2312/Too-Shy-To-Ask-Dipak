...
import com.app.quicklook.R
import com.app.quicklook.commonUtils.utility.storyUtil.PausableScaleAnimation
...
class PausableProgressBar @JvmOverloads constructor(
    ...
    fun setMaxWithoutCallback() {
        maxProgressView!!.setBackgroundResource(R.color.appColor)
        maxProgressView!!.visibility = View.VISIBLE
        if (animation != null) {
            animation!!.setAnimationListener(null)
            animation!!.cancel()
    ...
    private fun finishProgress(isMax: Boolean) {
        if (isMax) maxProgressView!!.setBackgroundResource(R.color.appColor)
